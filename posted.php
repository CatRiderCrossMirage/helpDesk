        ...
        <div class="row">
            <div class="col">
                <?php
                    ...
                    $sqlPost = "SELECT * FROM post WHERE id_post=2 LIMIT 1";

                    foreach($conn->query($sqlPost) as $row) {
                        echo '<h1>'. $row['topic']. '</h1><br>' ;
                        echo "<h5>".$row['content']. '</h5><br>'  ;
                        echo "<small class='text-muted'>ถามโดย : ".$row['writher']. "</small>"  ;
                        $idPost = $row['id_post'];

                    }

                    $sqlPic = "SELECT * FROM picture WHERE picture.id_post = $idPost";
                    
                    foreach($conn->query($sqlPic) as $row){
                        echo "<div id='images'><img src='images/{$row['img']}'></div>";
                    }
                ?>
            </div>
        </div>
